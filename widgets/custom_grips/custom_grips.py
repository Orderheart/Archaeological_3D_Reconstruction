...
# 负责创建自定义的可调整大小的控件（grip）。
# 这些控件允许用户拖动以调整父窗口的大小。具体来说，它根据给定的位置在父窗口的边缘创建一个调整大小的控件。
# 例如，可以在顶部、底部、左侧或右侧创建这样的控件。在初始化时，可以选择禁用控件的颜色，使其透明。
class CustomGrip(QWidget):
    # 它接受三个参数：parent表示父窗口，position表示控件在父窗口中的位置，disable_color表示是否禁用控件的颜色。
...
# 用于创建用于自定义调整大小控件的子部件。这些子部件包括顶部、底部、左侧和右侧的调整大小控件。
# 每个部件都具有特定的样式和功能，例如设置背景颜色、设置大小以及设置鼠标光标形状等
